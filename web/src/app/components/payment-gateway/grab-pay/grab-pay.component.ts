...
@Component({
  selector: 'app-grab-pay',
  templateUrl: './grab-pay.component.html',
  styleUrls: ['./grab-pay.component.scss']
})
export class GrabPayComponent implements OnInit {
  checkoutUrl: string;
  ...
  paymentSource() {
    ...
    var successUrl = `${environment.webUrl}/order/payment-success?orderid=${this.payment.orderId}&type=${this.payment.paymentType}&refid=${refid}`;
    var id = this.payment.Order.id;
    this.paymentIntentResource.attributes.type = 'grab_pay';
