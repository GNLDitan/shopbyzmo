...
export class PreOrderPaymentGuard implements CanActivate {
    ...
    canActivate(
        ...
        if (this.authenticationService.validateUser()) {
            return this.orderService.getPreOrderScheduleById(param).then((sched: PreOrderSchedule) => {
                if (sched != null) {
                    if (sched.isCleared) {
                        this.navigationService.toPaymentComplete(sched.id)
                    } else {
                        this.dataService.setPreOrderPayment(sched);
                        return true;
                    }
                } else {
